...
/**
 * Implementación del repositorio para gestionar el acceso a datos de la entidad Usuario
...
public class UsuarioRepositoryImpl implements UsuarioRepository {
    ...
    // Implementación de métodos de la interfaz UsuarioRepository
    ...
    public Optional<Usuario> findById(Integer id) {
        Usuario usuario = buscarPorId(id.longValue());
    ...
    public List<Usuario> findByActivo(boolean activo) {
        if (activo) {
            ...
            String sql = "SELECT * FROM usuario WHERE activo = false ORDER BY nombre ASC";
            return jdbcTemplate.query(sql, USUARIO_ROW_MAPPER);
        }
    }
    ...
    public Usuario save(Usuario usuario) {
        if (usuario.getId() != null && usuario.getId() > 0) {
            ...
            boolean actualizado = actualizar(usuario);
    ...
    public int updatePassword(Integer id, String password) {
        boolean actualizado = actualizarContrasena(id.longValue(), password);
        return actualizado ? 1 : 0;
    }

    @Override
    public int updateActivo(Integer id, boolean activo) {
        boolean actualizado = cambiarEstado(id.longValue(), activo);
        return actualizado ? 1 : 0;
    }
